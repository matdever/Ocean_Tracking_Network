...
%           - The glider moves through time, therefore never really
...
for tt = 1:length(U_dis)
    ...
    % method 3: take the median theorithical Udis during the glider
    % transect
    
    U_DIS_theo_v3(tt) = nanmedian(U_DIS_theory(start:finish));
    
    % method 2: take the minimum theorithical Udis during the glider
    % transect
...
for tt = 1:length(U_dis)
    if isnan(U_dis(tt))==0
        ...
        errorbar(U_DIS_theo_v2(tt),U_dis(tt),U_dis_err(tt),'color','k')
...
axis([-.3 0.001 -.3 0.001])
XL = get(gca,'xlim');
line(XL,XL,'color','k','linestyle','--')
%title('Comparison between predicted U_d_i_s and measured U_d_i_s')
SSE = nansum((U_dis - U_DIS_theo_v2).^2);
text(-0.28,-0.07,['SSE = ',num2str(SSE,'%3.3f')],'fontsize',18);
set(gca,'fontsize',18,...
    'ytick',[-.3:.1:.1],...
    'xtick',[-.3:.1:.1],'xaxislocation',...
    'bottom','yaxislocation','right')
text(-0.03,-0.27,'(b)','fontsize',18)

set(gcf,'color','w')
export_fig -r300 'Udis_validation.png'
